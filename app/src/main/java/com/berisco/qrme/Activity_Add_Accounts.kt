...
import kotlinx.android.synthetic.main.activity__add__accounts.*


class Activity_Add_Accounts : AppCompatActivity() {
...
//    var business_isEmpty :Any? =null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity__add__accounts)
...
//            }
        ...
        {
            Toast.makeText(this, "Missing Person First Name and Last Name", Toast.LENGTH_SHORT).show()
        }
...
//            var twitter = et_twitter.text
                    ...
                    Toast.makeText(applicationContext, "Data Save Successfully", Toast.LENGTH_LONG)
                        .show()
...
//                            et_discord.setText(discord.toString())
    ...
    var WebUrl = "^((ftp|http|https):\\/\\/)?(www.)?(?!.*(ftp|http|https|www.))[a-zA-Z0-9_-]+(\\.[a-zA-Z]+)+((\\/)[\\w#]+)*(\\/\\w+\\?[a-zA-Z0-9_]+=\\w+(&[a-zA-Z0-9_]+=\\w+)*)?$"
    private fun urlValidate(potentialUrl: String): Boolean {
        var website: String =(potentialUrl.trim()).toLowerCase() //txtWebsite.getText().toString().trim()
